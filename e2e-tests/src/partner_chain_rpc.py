    ...
    def __exec_rpc(self, method: str, params: Optional[list] = None) -> Dict[str, Any]:
        ...
        try:
            if os.environ.get("USE_KUBECTL_RPC") != "true":
                ...
                cmd.extend(["--", "curl", "-s", "-H", "Content-Type: application/json", "-d", payload, "http://localhost:9933"]) # Use curl within the pod
                result = subprocess.run(cmd, capture_output=True, text=True)
                if result.returncode != 0:
                    logger.error(f"kubectl exec command failed: {cmd}")
                    logger.error(f"stderr: {result.stderr}")
                    raise PartnerChainRpcException(f"Kubectl exec command failed: {result.stderr}", result.returncode)
                if not result.stdout.strip():
                    raise ValueError(f"Empty response from kubectl exec command: {cmd}")
                try:
                    return json.loads(result.stdout)
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse JSON response from kubectl exec: {e}")
                    logger.error(f"stdout: {result.stdout}")
                    raise PartnerChainRpcException(f"Invalid JSON response from kubectl exec: {e}")
