...
async fn get_epoch_committee() {
	...
	let candidate_data_source = MockAuthorityDataSource::default();
...
mod get_registration_tests {
	...
	async fn should_work() {
		...
		let candidate_data_source_mock = MockAuthorityDataSource::default()
	...
	async fn test_invalid_registration_data(
		...
		let mut candidate_data_source_mock = MockAuthorityDataSource::default()
			.with_candidates_per_epoch(vec![
				vec![],
