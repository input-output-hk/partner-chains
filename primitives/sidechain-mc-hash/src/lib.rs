...
#[cfg(feature = "std")]
pub use crate::inherent_provider::{McHashInherentDataProvider, McHashInherentDigest};
...
mod inherent_provider {
	use crate::{INHERENT_IDENTIFIER, MC_HASH_DIGEST_ID};
	...
	use McHashInherentError::StableBlockNotFound;
	...
	async fn get_mc_state_reference(
		...
		data_source
			.get_stable_block_for(
				verified_block_mc_hash.clone(),
				McTimestamp(timestamp.as_millis()),
			)
			.await
	...
	impl InherentDigest for McHashInherentDigest {
		...
		fn from_inherent_data(
			...
			let mc_hash = inherent_data
				...
				.map_err(|err| {
					format!("Failed to retrieve main chain hash from inherent data: {err}")
				})?
