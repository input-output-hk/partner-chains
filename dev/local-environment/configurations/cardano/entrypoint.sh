...
# --- SPO Registration and Delegation for Permissioned Nodes ---
echo "[LOG] Starting SPO registration and delegation for $NUM_PERMISSIONED_NODES_TO_PROCESS permissioned nodes..."

# Define stake pool deposit amount (from protocol parameters, usually 500 ADA = 500000000 lovelace)
POOL_DEPOSIT=500000000 # You should get this from protocol parameters preferably
STAKE_DEPOSIT=2000000 # 2 ADA for stake address registration
...
if [ -z "$UTXO_FUNDED_ADDRESS" ]; then
    echo "[DEBUG] CRITICAL ERROR: No UTXO found for funded address after main transaction. Cannot register SPOs. Aborting."
    exit 1
fi
...
# Ensure protocol parameters are available for fee calculation
protocol_params_file="/data/protocol.json" # Should already exist from main transaction fee calculation

# Loop through each permissioned node to register and delegate
for i in $(seq 0 $((NUM_PERMISSIONED_NODES_TO_PROCESS - 1))); do
    node_idx=$((i+1))
    NODE_SPECIFIC_KEYS_DIR="${permissioned_node_base_dirs[$i]}" # Reusing the base dir array

    echo "[LOG] Processing permissioned node $node_idx for SPO registration and delegation..."

    PAYMENT_SKEY="${NODE_SPECIFIC_KEYS_DIR}/payment.skey"
    STAKE_VKEY="${NODE_SPECIFIC_KEYS_DIR}/stake.vkey"
    STAKE_SKEY="${NODE_SPECIFIC_KEYS_DIR}/stake.skey"
    COLD_VKEY="${NODE_SPECIFIC_KEYS_DIR}/cold.vkey"
    COLD_SKEY="${NODE_SPECIFIC_KEYS_DIR}/cold.skey"
    VRF_VKEY="${NODE_SPECIFIC_KEYS_DIR}/vrf.vkey"
    NODE_COUNTER="${NODE_SPECIFIC_KEYS_DIR}/cold.counter"
    NODE_CERT="${NODE_SPECIFIC_KEYS_DIR}/node.cert"

    if [ ! -f "$PAYMENT_SKEY" ] || [ ! -f "$STAKE_VKEY" ] || [ ! -f "$STAKE_SKEY" ] || [ ! -f "$COLD_VKEY" ] || [ ! -f "$COLD_SKEY" ] || [ ! -f "$VRF_VKEY" ]; then
        echo "[DEBUG] Missing required key files for permissioned node $node_idx in $NODE_SPECIFIC_KEYS_DIR. Skipping SPO registration for this node."
        continue
    fi
     if [ ! -f "$NODE_COUNTER" ]; then
        echo "[DEBUG] Missing cold.counter file for permissioned node $node_idx in $NODE_SPECIFIC_KEYS_DIR. Attempting to create with value 0."
        echo 0 > "$NODE_COUNTER"
        if [ $? -ne 0 ]; then
            echo "[DEBUG] Failed to create cold.counter for node $node_idx. Skipping SPO registration."
            continue
        fi
    fi

    # 1. Create Stake Address Registration Certificate
    echo "[LOG] Creating stake address registration certificate for permissioned node $node_idx..."
    STAKE_REG_CERT="/data/stake_reg_node_${node_idx}.cert"
    cardano-cli latest stake address registration-certificate \\\
        --stake-verification-key-file "$STAKE_VKEY" \\\
        --out-file "$STAKE_REG_CERT"
    if [ $? -ne 0 ]; then echo "[DEBUG] Error creating stake registration certificate for node $node_idx. Skipping."; continue; fi

    # 2. Create Stake Delegation Certificate
    echo "[LOG] Creating stake delegation certificate for permissioned node $node_idx..."
    POOL_ID=$(cardano-cli latest stake-pool id --cold-verification-key-file "$COLD_VKEY")
    if [ -z "$POOL_ID" ]; then echo "[DEBUG] Error getting pool ID for node $node_idx. Skipping."; continue; fi
    STAKE_DEL_CERT="/data/stake_del_node_${node_idx}.cert"
    cardano-cli latest stake address delegation-certificate \\\
        --stake-verification-key-file "$STAKE_VKEY" \\\
        --stake-pool-id "$POOL_ID" \\\
        --out-file "$STAKE_DEL_CERT"
     if [ $? -ne 0 ]; then echo "[DEBUG] Error creating stake delegation certificate for node $node_idx. Skipping."; continue; fi

    # 3. Create Pool Registration Certificate
    echo "[LOG] Creating pool registration certificate for permissioned node $node_idx..."
    POOL_REG_CERT="/data/pool_reg_node_${node_idx}.cert"
    # Simplified pool registration - needs proper details for a real SPO
    cardano-cli latest stake-pool registration-certificate \\\
        ...
        --pool-cost 340000000 \\\
        --pool-margin 0.0 \\\
        --reward-account-verification-key-file "$STAKE_VKEY" \\\
        --pool-owner-stake-verification-key-file "$STAKE_VKEY" \\\
        --testnet-magic 42 \\\
        --out-file "$POOL_REG_CERT"
    if [ $? -ne 0 ]; then echo "[DEBUG] Error creating pool registration certificate for node $node_idx. Skipping."; continue; fi

    # 4. Create Node Operational Certificate
    echo "[LOG] Creating node operational certificate for permissioned node $node_idx..."
    cardano-cli latest node issue-opl-cert \\\
        --kes-verification-key-file "${NODE_SPECIFIC_KEYS_DIR}/kes.vkey" \\\
        --cold-signing-key-file "$COLD_SKEY" \\\
        --operational-certificate-issue-counter "$NODE_COUNTER" \\\
        --kes-period 0 \\\
        --out-file "$NODE_CERT"
    if [ $? -ne 0 ]; then echo "[DEBUG] Error creating node operational certificate for node $node_idx. Skipping."; continue; fi
    echo "[LOG] Node operational certificate created for node $node_idx at $NODE_CERT"

    # 5. Build the SPO transaction (contains stake reg, delegation, pool reg)
    echo "[LOG] Building SPO transaction for permissioned node $node_idx..."
    SPO_TX_RAW="/data/spo_tx_node_${node_idx}.raw"
    cardano-cli latest transaction build-raw \\\
        --tx-in "$TX_IN_SPO" \\\
        --tx-out "$new_address+$TX_IN_SPO_AMOUNT"  /* This will be updated with change */ \\
        --certificate "$STAKE_REG_CERT" \\\
        --certificate "$STAKE_DEL_CERT" \\\
        --certificate "$POOL_REG_CERT" \\\
        --fee 0 \\\
        --out-file "$SPO_TX_RAW"
    if [ $? -ne 0 ]; then echo "[DEBUG] Error building SPO transaction raw for node $node_idx. Skipping."; continue; fi

    # 6. Calculate Fee for the SPO transaction
    echo "[LOG] Calculating fee for SPO transaction for permissioned node $node_idx..."
    SPO_TX_NUM_INPUTS=1
    SPO_TX_NUM_OUTPUTS=1 # Just the change output for now
    SPO_TX_NUM_CERTIFICATES=3 # stake registration, stake delegation, pool registration
    SPO_TX_NUM_WITNESSES=4 # funded_address.skey, stake.skey, cold.skey, payment.skey (for change) - confirm witnesses needed

    CALCULATED_SPO_FEE=$(cardano-cli latest transaction calculate-min-fee \\\
        --tx-body-file "$SPO_TX_RAW" \\\
        --testnet-magic 42 \\\
        --protocol-params-file "$protocol_params_file" \\\
        --tx-in-count "$SPO_TX_NUM_INPUTS" \\\
        --tx-out-count "$SPO_TX_NUM_OUTPUTS" \\\
        --witness-count "$SPO_TX_NUM_WITNESSES" | /busybox awk \'{print $1}\')

    if ! [[ "$CALCULATED_SPO_FEE" =~ ^[0-9]+$ ]] || [ -z "$CALCULATED_SPO_FEE" ]; then
        echo "[DEBUG] ERROR calculating dynamic fee for SPO tx (Raw output: \'$CALCULATED_SPO_FEE\'). Using fallback static fee 500000."
        SPO_TX_FEE=500000
    else
        SPO_TX_FEE=$((CALCULATED_SPO_FEE + 1000)) # Add a 1000 lovelace buffer
        echo "[LOG] Calculated Min Fee for SPO tx: $CALCULATED_SPO_FEE, Using Fee with Buffer: $SPO_TX_FEE"
    fi

    # 7. Calculate Change and Rebuild the SPO transaction
    SPO_TX_CHANGE=$((TX_IN_SPO_AMOUNT - POOL_DEPOSIT - STAKE_DEPOSIT - SPO_TX_FEE))
    echo "[LOG] SPO Tx: Change calculated: $SPO_TX_CHANGE (TX_IN_SPO_AMOUNT=$TX_IN_SPO_AMOUNT - POOL_DEPOSIT=$POOL_DEPOSIT - STAKE_DEPOSIT=$STAKE_DEPOSIT - SPO_TX_FEE=$SPO_TX_FEE)"

    if [ "$SPO_TX_CHANGE" -lt 1000000 ]; then # Minimum change 1 ADA
        echo "[DEBUG] CRITICAL ERROR: SPO transaction change for node $node_idx is less than 1 ADA ($SPO_TX_CHANGE). Aborting SPO registration for this node."
        continue # Skip to the next node
    fi

    echo "[LOG] Rebuilding FINAL SPO transaction raw file for permissioned node $node_idx..."
    SPO_TX_RAW_FINAL="/data/spo_tx_node_${node_idx}_final.raw"
    cardano-cli latest transaction build-raw \\\
        --tx-in "$TX_IN_SPO" \\\
        --tx-out "$new_address+$SPO_TX_CHANGE" \\\
        --certificate "$STAKE_REG_CERT" \\\
        --certificate "$STAKE_DEL_CERT" \\\
        --certificate "$POOL_REG_CERT" \\\
        --fee "$SPO_TX_FEE" \\\
        --out-file "$SPO_TX_RAW_FINAL"
    if [ $? -ne 0 ]; then echo "[DEBUG] CRITICAL ERROR: Failed to rebuild FINAL SPO transaction for node $node_idx. Aborting SPO registration for this node."; continue; fi
    echo "[LOG] FINAL SPO transaction raw file created for node $node_idx at $SPO_TX_RAW_FINAL."

    # 8. Sign the SPO transaction
    echo "[LOG] Signing the SPO transaction for permissioned node $node_idx..."
    SPO_TX_SIGNED="/data/spo_tx_node_${node_idx}.signed"
    cardano-cli latest transaction sign \\\
        --tx-body-file "$SPO_TX_RAW_FINAL" \\\
        --signing-key-file /keys/funded_address.skey \\\
        --signing-key-file "$STAKE_SKEY" \\\
        --signing-key-file "$COLD_SKEY" \\\
        --signing-key-file "$PAYMENT_SKEY" \\\
        --testnet-magic 42 \\\
        --out-file "$SPO_TX_SIGNED"
    if [ $? -ne 0 ]; then echo "[DEBUG] CRITICAL ERROR: Failed to sign SPO transaction for node $node_idx. Aborting SPO registration for this node."; continue; fi
    echo "[LOG] SPO transaction signed for node $node_idx at $SPO_TX_SIGNED."

    # 9. Submit the SPO transaction
    echo "[LOG] Submitting the SPO transaction for permissioned node $node_idx..."
    if ! cardano-cli latest transaction submit \\\
        --tx-file "$SPO_TX_SIGNED" \\\
        --testnet-magic 42; then
        echo "[DEBUG] CRITICAL ERROR: Failed to submit SPO transaction for node $node_idx. Aborting SPO registration for this node."
        # Consider adding error handling here, like checking UTXO or querying transaction status
        continue
    fi
    echo "[LOG] SPO transaction submitted successfully for permissioned node $node_idx."

    # After successful submission, the UTXO of funded_address is spent.
    # Need to query again for the next iteration's TX_IN_SPO
    echo "[LOG] Re-querying UTXO for funded address: $new_address for the next SPO transaction."
    UTXO_FUNDED_ADDRESS=$(cardano-cli latest query utxo \\\
      --address "$new_address" \\\
      --testnet-magic 42 | /busybox tail -n +3) # Skip header

    if [ -z "$UTXO_FUNDED_ADDRESS" ]; then
        echo "[DEBUG] INFO: No UTXO found for funded address after submitting SPO transaction for node $node_idx. This is expected if this is the last node or if an error occurred. The loop will terminate."
        # Break loop if no UTXO is found, as we can't fund further transactions
        break
    fi
...
echo "[LOG] Completed SPO registration and delegation process for permissioned nodes."
# --- End of SPO Registration and Delegation ---
