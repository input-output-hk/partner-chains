...
choose_deployment_option() {
  ...
  if [[ $modify_stack =~ ^[Yy]$ ]]; then
    ...
    echo "4) Deploy Partner Chains node on host networking configured with full MC stack"
    echo "5) Deploy single Partner Chains node with no MC stack"
...
create_docker_compose() {
    ...
    case $deployment_option in
      ...
      5)
        ...
        echo -e "Ensure the latest chain-spec.json has been copied into local directory from Alice node\n"
