...
use sidechain_domain::{MainchainKeyHash, McEpochNumber};
...
pub mod inherent_data {
	...
	impl<BlockProducer, Beneficiary> BlockParticipationInherentDataProvider<BlockProducer, Beneficiary>
	where
		...
		BlockProducer: AsCardanoSPO,
	{
			...
			T::Api: BlockParticipationApi<Block, BlockProducer>,
		{
			...
			let mut production_rundowns = vec![];
			for (mc_epoch, producer_blocks) in block_counts_by_epoch_and_producer {
				...
				for (producer, block_count) in producer_blocks {
					...
					production_rundowns.push(breakdown);
				}
			}

			Ok(Self::Active {
				target_inherent_id,
				block_production_data: BlockProductionData::new(up_to_slot, production_rundowns),
			})
		}
		...
		fn production_breakdown_for(
			...
			BlockProducerParticipationData<BlockProducer, Beneficiary>,
			...
			let (beneficiary_total_share, beneficiaries) = match block_producer.as_cardano_spo() {
				...
				Some(cardano_producer) => {
					...
					let beneficiaries = delegators
						.iter()
						.map(|(delegator_id, stake_amount)| DelegatorBlockParticipationData {
							id: Beneficiary::from(delegator_id.clone()),
