...
pub fn legacy_governance_init_transaction(
	...
	let governance_authorithy_key_hash = PrivateKey::from_normal_bytes(&governance_authority_key)
		.unwrap()
		.to_public()
		.hash()
		.to_bytes();
	...
	let asset_name = AssetName::new(hex!("56657273696f6e206f7261636c65").to_vec()).unwrap();
	assets.insert(&asset_name, &1u32.into());
	gov_token_ma.insert(&version_oracle_script_hash, &assets);
	let mut output = TransactionOutput::new(
		&Address::from_bech32("addr_test1wplvesjjxtg8lhyy34ak2dr9l3kz8ged3hajvcvpanfx7rcwzvtc5")
			.unwrap(),
	...
	body.set_mint(&Mint::new_from_entry(
		...
		&MintAssets::new_from_entry(&asset_name, &Int::new_i32(1)).unwrap(),
	));
	...
	assert_eq!(version_oracle_plutus_script.hash(), version_oracle_script_hash);
	...
	vkeys.add(&Vkeywitness::new(&Vkey::new(&private_key.to_public()), &signature));
