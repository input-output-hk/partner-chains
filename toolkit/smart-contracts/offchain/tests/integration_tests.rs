...
async fn upsert_d_param() {
	...
	let genesis_utxo = run_init_governance(&client).await;
	assert!(
		run_upsert_d_param(genesis_utxo, 0, 1, 2, &governance_authority_payment_key(), &client)
			.await
			.is_some()
	);
	assert!(
	...
	assert_eq!(
		...
		DParameter {
			...
			num_native_stake_candidates: 2,
		}
	);
	assert!(
		run_upsert_d_param(genesis_utxo, 1, 1, 1, &governance_authority_payment_key(), &client)
			.await
			.is_some()
	)
}
