...
pub enum DParamDatum {
	...
	/// Schema with native stake validators
	V1 {
		/// Number of permissioned candidates in committee.
		num_permissioned_candidates: u16,
		/// Number of registered candidates in committee.
		num_registered_candidates: u16,
		/// Number of native stake validators in committee.
...
/// Decodes V1 appendix of D-param.
pub fn decode_v1_appendix(data: &PlutusData) -> Result<DParamDatum, String> {
	let d_parameter = data
		.as_list()
		.filter(|datum| datum.len() == 3)
		.and_then(|items| {
...
impl VersionedDatumWithLegacy for DParamDatum {
	...
	fn decode_versioned(
		...
		match version {
			...
			1 => DParamDatum::decode_v1_appendix(appendix)
				.map_err(|msg| format!("Cannot parse appendix: {msg}")),
...
mod tests {
	...
	fn domain_d_param_to_csl() {
		let d_param = sidechain_domain::DParameter {
			...
			num_native_stake_candidates: 23,
		};
	...
	fn valid_v1_d_param() {
		...
		let expected_datum = DParamDatum::V1 {
			num_permissioned_candidates: 17,
			num_registered_candidates: 42,
			num_native_stake_candidates: 13,
		};
