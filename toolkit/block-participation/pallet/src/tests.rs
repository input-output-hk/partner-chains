...
use sp_block_participation::INHERENT_IDENTIFIER;
use sp_runtime::BoundedVec;

#[test]
fn inherent_clears_production_log_prefix() {
	new_test_ext().execute_with(|| {
		let initial_log = (1..=20).map(|i| (i, i + 100)).collect();
		let expected_log: Vec<_> = (11..=20).map(|i| (i, i + 100)).collect();
		mock_pallet::BlockProductionLog::<Test>::put(BoundedVec::truncate_from(initial_log));

		assert_eq!(ProcessedUpTo::<Test>::get(), 0);
		Payouts::note_processing(Origin::<Test>::None.into(), 10).expect("Should succeed");

		let log = mock_pallet::BlockProductionLog::<Test>::get().unwrap().to_vec();
		assert_eq!(log, expected_log);
		assert_eq!(ProcessedUpTo::<Test>::get(), 10);
	})
}
...
fn inherent_cant_be_run_twice_in_one_block() {
	new_test_ext().execute_with(|| {
		Payouts::note_processing(Origin::<Test>::None.into(), 10)
...
fn creates_inherent_if_data_present() {
	...
	inherent_data.put_data(INHERENT_IDENTIFIER, &24u64).unwrap();

	let inherent = Payouts::create_inherent(&inherent_data).expect("Should crate an inherent");

	assert_eq!(inherent, crate::Call::<Test>::note_processing { up_to_moment: 24 })
}

#[test]
fn check_fails_if_data_is_missing() {
	let inherent_data = InherentData::new();
	let inherent = crate::Call::<Test>::note_processing { up_to_moment: 25 };
...
fn check_fails_if_slot_is_different() {
	...
	let result = Payouts::check_inherent(&inherent, &inherent_data);
	assert_eq!(result, Err(sp_block_participation::InherentError::InvalidInherentData))
}
