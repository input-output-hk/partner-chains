	...
	ConvertKeys: MaybeFromCandidateKeys<TAccountKeys>,
{
	...
	Ok((
		ecdsa_bytes.into(),
		ConvertKeys::to_opaque_keys(&candidate.keys)
...
pub fn validate_registration_data<TAccountKeys, X>(
	stake_pool_pub_key: &StakePoolPublicKey,
	registration_data: &RegistrationData,
	genesis_utxo: UtxoId,
) -> Result<(ecdsa::Public, TAccountKeys), RegistrationDataError>
where
	X: MaybeFromCandidateKeys<TAccountKeys>,
{
	let account_keys = X::to_opaque_keys(&registration_data.keys)
