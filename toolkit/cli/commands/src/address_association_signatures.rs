...
> {
	...
	/// Cardano address to be associated with the Partner Chain address
	#[arg(long)]
	...
	///
	/// This key MUST correspond to the Cardano address passed.
	#[clap(flatten)]
	...
	PartnerchainAddress: Serialize + Clone + Sync + Send + FromStr + Encode + 'static,
{
	pub fn execute(&self) -> anyhow::Result<()> {
		let Self { mainchain_address, partnerchain_address, genesis_utxo, signing_key_file } =
			self.clone();
		let key = signing_key_file.read_key().map_err(|e| anyhow!(e))?;
		let msg = AddressAssociationSignedMessage {
			mainchain_address: mainchain_address.clone(),
