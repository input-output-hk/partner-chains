...
fn happy_path() {
	...
	let context = MockIOContext::new()
		...
		.with_expected_io(vec![
			...
			MockIO::eprint(&format!(
				"🛠️ Loaded Cardano data source from config ({RESOURCES_CONFIG_FILE_PATH}): {CARDANO_DATA_SOURCE}"
			)),
			MockIO::eprint(&format!(
...
mod data_source_config {
	...
	use super::*;
	use crate::{
		start_node::DataSourceConfig,
		tests::{MockIO, MockIOContext, RESOURCES_CONFIG_FILE_PATH},
	};

	#[test]
	fn incorrect_data_source() {
		let data_source = "db-syn";

		let resources_config = serde_json::json!({
			"substrate_node_base_path": DATA_PATH,
			"db_sync_postgres_connection_string": DB_CONNECTION_STRING,
		...
		let context = MockIOContext::new()
			.with_json_file(RESOURCES_CONFIG_FILE_PATH, resources_config)
			.with_expected_io(vec![MockIO::eprint(
				"⚠️ Invalid Cardano data source. Supported values are: db-sync",
			)]);

		let result = DataSourceConfig::load(&data_source, &context);
