	...
	pub fn generate_all_spo_keys(
		...
		MockIO::Group(vec![
			...
			MockIO::run_command_json(
				&format!("<mock executable> key generate --scheme ecdsa --output-type json"),
			...
			MockIO::run_command(
				&format!(
					"<mock executable> key insert --base-path {DATA_PATH} --scheme ecdsa --key-type crch --suri 'cross-chain secret phrase'"
				),
				"",
			),
			...
			MockIO::run_command_json(
				&format!("<mock executable> key generate --scheme ed25519 --output-type json"),
				&serde_json::json!({"publicKey": grandpa_key, "secretPhrase": "grandpa secret phrase"}),
			),
			MockIO::eprint("💾 Inserting Grandpa (ed25519) key"),
			MockIO::run_command(
				&format!(
					"<mock executable> key insert --base-path {DATA_PATH} --scheme ed25519 --key-type gran --suri 'grandpa secret phrase'"
				),
				"",
			),
			MockIO::eprint(&format!(
			...
			MockIO::run_command_json(
				&format!("<mock executable> key generate --scheme sr25519 --output-type json"),
				&serde_json::json!({"publicKey": aura_key, "secretPhrase": "aura secret phrase"}),
			),
			MockIO::eprint("💾 Inserting Aura (sr25519) key"),
			MockIO::run_command(
				&format!(
					"<mock executable> key insert --base-path {DATA_PATH} --scheme sr25519 --key-type aura --suri 'aura secret phrase'"
				),
				"",
			),
			MockIO::eprint(&format!(
