...
use subxt::{OnlineClient, SubstrateConfig, dynamic::Value, metadata::{Metadata, TypeDef}};
use tokio;
...
async fn test_generate_keys_with_valid_response() {
    ...
    mock_subxt_client.expect_from_url()
        .once()
        .with(eq("http://localhost:9933"))
        .returning(|_| Ok(MockOnlineClient::new()));
    mock_subxt_client.expect_metadata()
        .once()
        .returning(|| {
            ...
            metadata.types_mut().register_type(
                ...
                scale_type_resolver::Type {
                    path: scale_type_resolver::TypePath::from_segments(["pallet_session", "SessionKeys"]).unwrap(),
            ...
            metadata.types_mut().register_type(
                1,
                scale_type_resolver::Type {
                    path: scale_type_resolver::TypePath::from_segments(["sp_consensus_aura", "sr25519", "app", "Public"]).unwrap(),
            ...
            metadata.types_mut().register_type(
                2,
                scale_type_resolver::Type {
                    path: scale_type_resolver::TypePath::from_segments(["sp_finality_grandpa", "app", "Public"]).unwrap(),
                    type_def: TypeDef::Composite(scale_type_resolver::Composite::new(vec![])),
                    docs: vec![],
                },
            );
    ...
    mock_client_builder.expect_build()
        .once()
