...
fn prepare_mc_follower_env<C: IOContext>(context: &C) -> anyhow::Result<()> {
	let data_source =
		config_fields::CARDANO_DATA_SOURCE.prompt_with_default_from_file_and_save(context);
	...
	set_data_sources_env(context, &chain_config.cardano, &postgres_connection_string, &data_source);
	Ok(())
}
...
mod tests {
	...
	fn show_registration_status_io() -> Vec<MockIO> {
		vec![
			...
			MockIO::prompt(
				"Enter the Cardano data source",
				CARDANO_DATA_SOURCE.default,
				CARDANO_DATA_SOURCE.default.unwrap(),
			),
			MockIO::prompt(
			...
			MockIO::set_env_var("CARDANO_DATA_SOURCE", "db-sync"),
