...
use midnight_circuits::instructions::SpongeCPU;
	...
	pub fn msg_from_bytes(msg: &[u8], format_scalars: bool) -> Result<Vec<Scalar>, PoseidonError> {
		...
		msg.chunks(chunk_len)
			.map(|scalar| {
				...
				Scalar::from_bytes_le(&bytes).into_option().ok_or(PoseidonError::NotCanonical)
			})
...
impl Hasher for PoseidonJubjub {
	...
	fn hash(s: &[u8]) -> Self::Out {
		...
		let mut hasher = <PoseidonChip<Scalar> as SpongeCPU<Scalar, Scalar>>::init(Some(msg.len()));
		<PoseidonChip<Scalar> as SpongeCPU<Scalar, Scalar>>::absorb(&mut hasher, &msg);

		let out_fr = <PoseidonChip<Scalar> as SpongeCPU<Scalar, Scalar>>::squeeze(&mut hasher);

		out_fr.to_bytes_le().into()
	}
}
