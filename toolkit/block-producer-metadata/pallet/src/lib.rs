...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn upsert_metadata(
			...
			let is_valid_signature =
				signature.verify(&cross_chain_pub_key.into(), &metadata_message.encode());
	...
	impl<T: Config> Pallet<T> {
		...
		) -> impl Iterator<Item = (CrossChainKeyHash, T::BlockProducerMetadata)> {
			BlockProducerMetadataStorage::<T>::iter()
		}
