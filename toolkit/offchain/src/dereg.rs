...
use crate::{csl::key_ed25519_hash_address, plutus_script::PlutusScript, tx};
use anyhow::anyhow;
use cardano_serialization_lib::{ExUnits, NetworkIdKind, PlutusData};
...
use partner_chains_plutus_data::registered_candidates::{
	decode_legacy_register_validator_datum, RegisterValidatorDatum,
};
...
pub async fn deregister(network: NetworkIdKind) -> anyhow::Result<TransactionHash> {
	...
	let applied_validator = PlutusScript::from_wrapped_cbor(&CANDIDATE_VALIDATOR_CBOR)?
		.apply_data(sidechain_params())?;

	//validator_address: "addr_test1wq7vcwawqa29a5a2z7q8qs6k0cuvp6z2puvd8xx7vasuajq86paxz"
	let validator_address = applied_validator.plutus_v2_address(network);
	...
	validator_utxos
		...
		.filter(|utxo| {
			...
			let rd_opt: Option<RegisterValidatorDatum> = datum
				...
				.and_then(|pd: PlutusData| decode_legacy_register_validator_datum(&pd));
			match rd_opt {
				Some(RegisterValidatorDatum::V0 {
					stake_ownership,
					own_pkh: datum_own_pkh,
					..
				}) => &datum_own_pkh.0 == own_pkh && &stake_ownership.pub_key == spo_pub_key,
				_ => false,
			}
		})
		.collect()
