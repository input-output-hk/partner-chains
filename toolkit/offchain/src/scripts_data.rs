...
pub fn get_scripts_data(
	...
	let committee_candidate_validator =
		PlutusScript::from_wrapped_cbor(raw_scripts::COMMITTEE_CANDIDATE_VALIDATOR)?
			.apply_data(pc_params.clone())?;
	let d_parameter_validator =
		PlutusScript::from_wrapped_cbor(raw_scripts::D_PARAMETER_VALIDATOR)?
			.apply_data(pc_params.clone())?;
	let illiquid_circulation_supply_validator =
		PlutusScript::from_wrapped_cbor(raw_scripts::ILLIQUID_CIRCULATION_SUPPLY_VALIDATOR)?
			.apply_uplc_data(version_oracle_policy_data.clone())?;
	let permissioned_candidates_validator =
		PlutusScript::from_wrapped_cbor(raw_scripts::PERMISSIONED_CANDIDATES_VALIDATOR)?
			.apply_data(pc_params.clone())?;
	let reserve_validator = PlutusScript::from_wrapped_cbor(raw_scripts::RESERVE_VALIDATOR)?
		.apply_uplc_data(version_oracle_policy_data.clone())?;

	let d_parameter_policy = PlutusScript::from_wrapped_cbor(raw_scripts::D_PARAMETER_POLICY)?
		.apply_data(pc_params.clone())?
		.apply_uplc_data(d_parameter_validator.plutus_v2_address_data(network)?)?;
	let permissioned_candidates_policy =
		PlutusScript::from_wrapped_cbor(raw_scripts::PERMISSIONED_CANDIDATES_POLICY)?
			.apply_data(pc_params.clone())?
			.apply_uplc_data(permissioned_candidates_validator.plutus_v2_address_data(network)?)?;
	let governance_policy = PlutusScript::from_wrapped_cbor(raw_scripts::MULTI_SIG_POLICY)?
		.apply_uplc_data(multisig_governance_policy_configuration(pc_params.clone()))?;
	let init_token_policy = PlutusScript::from_wrapped_cbor(raw_scripts::INIT_TOKEN_POLICY)?
		.apply_data(pc_params.clone())?;
	let reserve_auth_policy = PlutusScript::from_wrapped_cbor(raw_scripts::RESERVE_AUTH_POLICY)?
		.apply_uplc_data(version_oracle_policy_data)?;

	Ok(ScriptsData {
		addresses: Addresses {
			committee_candidate_validator: committee_candidate_validator
				.plutus_v2_address_bech32(network)?,
			d_parameter_validator: d_parameter_validator.plutus_v2_address_bech32(network)?,
			...
			permissioned_candidates_validator: permissioned_candidates_validator
				.plutus_v2_address_bech32(network)?,
			reserve_validator: reserve_validator.plutus_v2_address_bech32(network)?,
			version_oracle_validator: version_oracle_validator.plutus_v2_address_bech32(network)?,
		},
		validator_hashes: ValidatorHashes {
			committee_candidate_validator: committee_candidate_validator.script_plutus_v2_address(),
			d_parameter_validator: d_parameter_validator.script_plutus_v2_address(),
			illiquid_circulation_supply_validator: illiquid_circulation_supply_validator
				.script_plutus_v2_address(),
			permissioned_candidates_validator: permissioned_candidates_validator
				.script_plutus_v2_address(),
			reserve_validator: reserve_validator.script_plutus_v2_address(),
			version_oracle_validator: version_oracle_validator.script_plutus_v2_address(),
		},
		policy_ids: PolicyIds {
			d_parameter: d_parameter_policy.plutus_v2_policy_id(),
			init_token: init_token_policy.plutus_v2_policy_id(),
			governance: governance_policy.plutus_v2_policy_id(),
			permissioned_candidates: permissioned_candidates_policy.plutus_v2_policy_id(),
			reserve_auth: reserve_auth_policy.plutus_v2_policy_id(),
...
fn version_oracle(
	...
	let validator = PlutusScript::from_wrapped_cbor(raw_scripts::VERSION_ORACLE_VALIDATOR)?
		.apply_data(pc_params.clone())?;
	let policy_script = PlutusScript::from_wrapped_cbor(raw_scripts::VERSION_ORACLE_POLICY)?
		.apply_data(pc_params.clone())?
		.apply_uplc_data(init_token_asset_data)?
		.apply_uplc_data(validator.plutus_v2_address_data(network)?)?;
	let policy = policy_script.plutus_v2_policy_id();
