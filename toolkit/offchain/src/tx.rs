...
use cardano_serialization_lib::{
	...
	Redeemer, RedeemerTag, Transaction, TransactionBuilder, TxInputsBuilder, Value, Vkey,
	Vkeywitness, Vkeywitnesses,
};
use ogmios_client::{query_ledger_state::ProtocolParametersResponse, types::OgmiosUtxo};

use crate::csl::utxo_to_tx_input;
...
pub fn make_deregister_tx(
	...
	for utxo in own_registration_utxos.iter() {
		let amount: BigNum = crate::csl::convert_value(&utxo.value)?.coin();
		let input = utxo_to_tx_input(utxo);
		tx_inputs_builder.add_key_input(&From::from(own_pkh), &input, &Value::new(&amount));
		...
		//                      ex_units - they are zero before sending the transaction for evaluation, in the subsequent call it should be set with evaluation result
		...
		// This input is not present in the build transaction body but as a witness (redeemer above as well).
		...
		tx_inputs_builder.add_plutus_script_input(
			&PlutusWitness::new_without_datum(
				&PlutusScript::new_v2(validator_bytes.to_vec()),
				&redeemer,
			),
			&input,
			&Value::new(&amount),
	...
	let collateral_builder = get_collateral_builder(collaterals, own_pkh);
