...
use sidechain_domain::{MainchainKeyHash, McTxHash, UtxoId, UtxoIndex, McSmartContractResult, McSmartContractResult::{TxHash, TxCBOR}};
...
>(
	...
	let tx = Costs::calculate_costs(
		|costs| {
			update_governance_tx(
				...
				genesis_utxo_id,
				old_governance_authority.clone(),
				new_governance_authority.clone(),
	...
	if old_governance_authority == vec![context_pub_key_hash] {
...
fn update_governance_tx(
	...
	let multi_sig_policy =
		PlutusScript::from_wrapped_cbor(multi_sig_policy, Language::new_plutus_v2())?
			.apply_uplc_data(multisig_governance_policy_configuration(new_governance_authority, new_governance_threshold))?;
	...
	for key in old_governance_authority.iter() {
