...
impl PlutusScript {
	pub fn from_cbor(cbor: &[u8]) -> Self {
		Self { bytes: cbor.into() }
	}
	pub fn from_wrapped_cbor(cbor: &[u8]) -> anyhow::Result<Self> {
		Ok(Self::from_cbor(&unwrap_one_layer_of_cbor(cbor)?))
	}
	...
	pub fn plutus_v2_address(&self, network: NetworkIdKind) -> Address {
		plutus_script_address(&self.bytes, network, LanguageKind::PlutusV2)
	}
	...
	pub fn plutus_v2_address_data(
		&self,
		network: NetworkIdKind,
	) -> anyhow::Result<uplc::PlutusData> {
		csl_plutus_data_to_uplc(&PlutusData::from_address(&self.plutus_v2_address(network))?)
	}

	/// Returns bech32 address of the given PlutusV2 script
	pub fn plutus_v2_address_bech32(&self, network: NetworkIdKind) -> anyhow::Result<String> {
		self.plutus_v2_address(network)
			.to_bech32(None)
			.context("Converting script address to bech32")
	}

	pub fn script_plutus_v2_hash(&self) -> [u8; 28] {
		plutus_script_hash(&self.bytes, LanguageKind::PlutusV2)
	}

	pub fn script_plutus_v2_address(&self) -> MainchainAddressHash {
		MainchainAddressHash(self.script_plutus_v2_hash())
	}

	pub fn plutus_v2_policy_id(&self) -> PolicyId {
		PolicyId(self.script_plutus_v2_hash())
	}
}
