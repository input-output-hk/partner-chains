...
pub enum DataSourceError {
	...
	#[error(
		"Could not decode {datum:?} to {to:?}, this means that there is an error in Plutus scripts or chain-follower is obsolete."
	)]
	DatumDecodeError { datum: PlutusData, to: String },
	#[error(
		"'{0}' not found. Possible causes: main chain follower configuration error, db-sync not synced fully, or data not set on the main chain."
	)]
	ExpectedDataNotFound(String),
	#[error(
		"Invalid data. {0} Possible cause it an error in Plutus scripts or chain-follower is obsolete."
	)]
	InvalidData(String),
}
