...
#[derive(new)]
pub struct SessionValidatorManagementRpc<
	C,
	Block,
...
impl<
		...
		CrossChainPublic: parity_scale_codec::Decode
			...
			+ Sync
			+ 'static,
	> SessionValidatorManagementRpcApiServer
	for SessionValidatorManagementRpc<C, Block, SessionKeys, CrossChainPublic, SidechainParams>
where
	Block: BlockT,
	...
	C::Api: CandidateValidationApi<Block>,
{
	fn get_epoch_committee(&self, epoch_number: u64) -> RpcResult<GetCommitteeResponse> {
		...
		let first_epoch = {
			let second_block = (self.client)
				.hash(1.into())
				.map_err(error_object_from)?
				.ok_or(error_object_from("Only the Genesis Block exists at the moment!"))?;
		...
		if epoch_number < first_epoch {
			return Err(error_object_from_str(format!(
		...
		let epoch_of_latest_block =
			self.client.get_epoch_of_block(latest_block).map_err(error_object_from)?;

		if epoch_number > epoch_of_latest_block.next() {
			return Err(error_object_from_str(format!(
				"Committee is unknown for epoch {epoch_number}"
			)));
		}
		...
		let (_, committee) = if epoch_number == epoch_of_latest_block.next() {
			(self.client.runtime_api())
				...
				.map_err(error_object_from)?
				.ok_or(error_object_from_str(format!(
			...
			let block_hash = self
				...
				.map_err(error_object_from)?;
			(self.client.runtime_api())
				.get_current_committee(block_hash)
				.map_err(error_object_from)
		}?;
	...
	async fn get_registrations(
		...
		let scripts = api.get_main_chain_scripts(best_block).map_err(error_object_from)?;
			...
			permissioned_candidates: ariadne_parameters_response
				...
				.map(|candidate| {
					let validation_result =
						validate_permissioned_candidate(&candidate).map_err(error_object_from)?;
					Ok::<PermissionedCandidateData, ErrorObjectOwned>(
						PermissionedCandidateData::new(candidate, validation_result),
					)
				})
...
pub fn error_object_from_str(msg: impl Into<String>) -> ErrorObjectOwned {
